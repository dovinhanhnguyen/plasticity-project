...
int main() {
	...
	for (long i = INITIAL_TIMESTEP; i <= INITIAL_TIMESTEP+SHEAR_TIMESTEPS; i += NEVERY) { // parsing SHEAR_NUMBER/NEVERY of files, starting with INITIAL_TIMESTEP file
		...
		if (fileIn.good()) {
			...
			dataHandler.modify_contact_matrix(INITIAL_TIMESTEP, dataHandler.Box_Bounds[3], YMAX, VELOCITY_LAYERS, NUMBER_OF_CUTOFF_LAYERS); //TO-DO
			dataHandler.compare_contact_matrix(); //TO-DO
